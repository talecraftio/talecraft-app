...
def main():
    ...
    game1 = Game2.deploy(Resource[-1].address, PHI[-1].address, lending.address, 10000000000000000000, 6, 50, {'from': deployer})
    ...
    game2 = Game2.deploy(Resource[-1].address, PHI[-1].address, lending.address, 12000000000000000000, 51, 150, {'from': deployer})
    games['1'] = game2.address
    game2.transferOwnership('0xd4AE6402155Ec508C6Ca7Dd833fd355c6eDd1c14', {'from': deployer})

    game3 = Game2.deploy(Resource[-1].address, PHI[-1].address, lending.address, 15000000000000000000, 151, 1000, {'from': deployer})
